    ...
    def new_curve(self):
        ...
        if not self.first_curve:
            ...
            if (not self.new_child or self.original_parent):
                ...
                self.points[0][1] = (self.points[0][0][0] + (dx * abs(self.path[-2][0] - self.path[-1][0]))*(2/2),
                                     self.points[0][0][1] + (dy * abs(self.path[-2][1] - self.path[-1][1]))*(2/2))
            else:
        ...
        self.predicted_curve = predict_curve(self.points)

        pass

    def advance_frame(self):

        for index, segment in enumerate(self.predicted_curve):
            if index != 0:
                pygame.draw.line(screen, white, segment, self.predicted_curve[index - 1])
