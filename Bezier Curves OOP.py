...
class Line:
    def __init__(self, first_curve = True, new_child = True, last_point = False, shared_point = False, original_parent = True, line_width = 5, decay = 0.008):
        self.path = []
        self.points = [[]]
    ...
    def new_curve(self):
        ...
        if self.original_parent:
            self.points[0][0] = ((width + (width+100) * random.randint(-1,1))/2, (height + (height+100) * random.randint(-1,1))/2)
        
        self.points[0].append((random.randint(width/4,width*3/4),random.randint(height/4,height*3/4)))
       
       #rotationmode
        if rotation_mode and len(lines) != 0 and not self.original_parent:
            if self.new_child:
            ...
            for index, point in enumerate(lines[0].points[0]):
                self.points[0][index] = ((math.cos(self.angle)*(point[0]-width/2) - math.sin(self.angle)*(point[1]-height/2))+width/2,
                                         (math.sin(self.angle)*(point[0]-width/2) + math.cos(self.angle)*(point[1]-height/2))+height/2)
        #start rotation
        if random.randint(0,5) == 0 and self.original_parent:
        ...
        if not self.first_curve:
            ...
            if not self.new_child:
                ...
                dx = math.sqrt(length**2 / (gradient**2 + 1)) * ((self.path[-1][0] - self.path[-2][0]) / abs(self.path[-1][0] - self.path[-2][0]))
                dy = math.sqrt(length**2 - dx**2) * ((self.path[-1][1] - self.path[-2][1]) / abs(self.path[-1][1] - self.path[-2][1]))
        ...
        if len(lines) < 25 and random.randint(0,0) == 0 and not self.first_curve and not self.new_child and id(self) not in death_list and not (self.schedule_death or self.die) and not rotation_mode:
            lines.append(Line(False, True, self.last_point, self.points[0][1], False))

        #Child tragically falls ill and will die soon
        if random.randint(0,1) == 0 and not self.original_parent and not self.new_child and not (self.schedule_death or self.die) and id(self) not in death_list and not rotation_mode:
            id_list = [id(i) for i in lines]
            self.grave_line = id(self)
            attempts = 0
            close = False
            while self.grave_line == id(self) or lines[id_list.index(self.grave_line)].schedule_death or lines[id_list.index(self.grave_line)].die or self.grave_line in death_list:
    ...
    def advance_frame(self):
        ...
        for line in range(1,len(self.points[0])):
            for point in range(len(self.points[line])):
                self.points[line][point] = (self.points[line - 1][point][0] + (self.points[line - 1][point + 1][0] - self.points[line - 1][point][0]) * self.percent , self.points[line - 1][point][1] + (self.points[line - 1][point + 1][1] - self.points[line - 1][point][1]) * self.percent)
                ...
                if self.mirror:
                    for transform in transformations:
                        pygame.draw.line(screen,self.drawing_colours[i - 1],
                                           (width/2 - (self.path[i][0]-width/2)*transform[0] , width/2 - (self.path[i][1]-width/2)*transform[1]),
                                           (width/2 - (self.path[i - 1][0]-width/2)*transform[0] , width/2 - (self.path[i - 1][1]-width/2)*transform[1]),self.line_width)
                       
                        pygame.draw.line(screen,self.drawing_colours[i - 1],
                                           (width/2 - (self.path[i][1]-width/2)*transform[1] , width/2 - (self.path[i][0]-width/2)*transform[0]),
                                           (width/2 - (self.path[i - 1][1]-width/2)*transform[1] , width/2 - (self.path[i - 1][0]-width/2)*transform[0]),self.line_width)
                else:  
                    pygame.draw.line(screen,self.drawing_colours[i - 1],self.path[i],self.path[i - 1],self.line_width)
